...
if (isset($_POST['type'])) {
    ...
    if ($_POST['type'] == "remaining_quota") {
        ...
        if (!$conn = $db->connect()) {
            ...
            $stmt = $conn->prepare("SELECT COALESCE(SUM(amount),0) as total_amount FROM `record` WHERE  week(`record`.`timestamp`)=week(now()) AND `status`!=2 AND `vid` ='$vid' ");
            $stmt->execute();
            if ($result = $stmt->fetchAll(PDO::FETCH_ASSOC)) {
    ...
    if ($_POST['type'] == "add_special_qr") {
        ...
        $db = new DbConnect;
        $sql = "INSERT INTO `special_qr`(`cid`, `purpose`, `amount`, `approval`, `ref`,`fid`, `qr_code`)  VALUES ('$cid','$purpose','$amount','$approval','$ref','$fid', '$qr_code');";
        echo $sql;
    ...
    if ($_POST['type'] == "remove_special_qr") {
        ...
        $db = new DbConnect;
        $conn = $db->connect();
        $stmt = $conn->prepare("UPDATE `special_qr` SET `status`=1 WHERE `sqr_id` ='$SPID' ");
        $stmt->execute();
        $myObj3 = new \stdClass();
        $myObj3->Status = "1";
        $myJSON3 = json_encode($myObj3);
