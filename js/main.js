...
function loadStation() {
  $.ajax({
    ...
    $("#tblStations").empty();
    $("#tblStations").append(
      "<thead><th>Station ID</th><th>Name</th><th>Reg No</th><th>Location</th><th>Phone</th><th>Open/Close Status</th><th>Queue Length</th></thead><tbody>"
    );
    result.forEach(function (result) {
      $("#tblStations").append(
        "<tr onclick=\"window.location.href = '" +
          getReDirectURL(result.sid, STATION) +
          "';\"><td>" +
          result.sid +
          "</td><td>" +
          result.name +
...
function loadCustomer() {
  $.ajax({
    ...
    result.forEach(function (result) {
      $("#tblCustomers").append(
        "<tr onclick=\"window.location.href = '" +
          getReDirectURL(result.cid, CUSTOMER) +
          "';\"><td>" +
          result.cid +
          "</td><td>" +
          result.name +
...
function loadVehicle() {
  $.ajax({
    ...
    result.forEach(function (result) {
      $("#tblVehicles").append(
        "<tr onclick=\"window.location.href = '" +
          getReDirectURL(result.vid, VEHICLE) +
          "';\"><td>" +
          result.reg_no +
          "</td><td>" +
          result.type +
...
function viewfulequota() {
  $.ajax({
    ...
    $("#quotaform").append(
      '<button type="button" onclick="updateQuota()" class="btn btn-success btn-icon-text float-right"> Update <i class="mdi mdi-check-all btn-icon-append"></i></button>'
    );
  });
}
...
function loadComplaints() {
  $.ajax({
    ...
    $("#tblComplaints").append(
      "<thead><th>Complaint ID</th><th>Date Time</th><th>Customer</th><th>Description</th></thead><tbody>"
    );
    result.forEach(function (result) {
      $("#tblComplaints").append(
        "<tr><td>" +
...
function extendBadge(sts, vlv) {
  ...
  if (sts == 0) {
    bg =
      "<button onclick='extendAccept(this.value)' class='btn btn-sm bg-warning' value=" +
      vlv +
      ">Activate</button>";
  } else if (sts == 1) {
...
function cancellationBadge(sts, vlv) {
  ...
  if (sts == 1) {
    bg =
      "<button onclick='cancelRecord(this.value)' class='btn btn-sm bg-danger' value=" +
      vlv +
      ">Cancel</button>";
  } else if (sts == 2) {
...
function specialQRBadge(sts, vlv) {
  ...
  if (sts == 0) {
    bg =
      "<button onclick='specialQRAccept(this.value)' class='btn btn-sm bg-warning' value=" +
      vlv +
      ">Activate</button>";
  } else if (sts == 1) {
    bg = "<span class='badge badge-sm badge-success'>Active</span>";
...
function bowserBadge(ID, sts) {
  ...
  var item =
    "<select onchange='bowserAccept(" +
    ID +
    ",this)'><option value=0 " +
    op0 +
    ">Pending</option><option value=1 " +
    op1 +
    ">On Route</option><option value=2 " +
    op2 +
    ">Arrived</option><option value=3 " +
    op3 +
    ">Will be delayed</option></select>";
  return item;
}
...
function getReportData() {
  $.ajax({
    ...
    result.forEach(function (result) {
      row = [
        result.week,
        result.petrol ? Number(result.petrol) : 0,
        result.superPetrol ? Number(result.superPetrol) : 0,
        result.diesel ? Number(result.diesel) : 0,
        result.superDiesel ? Number(result.superDiesel) : 0,
      ];
      statData.push(row);
    });
